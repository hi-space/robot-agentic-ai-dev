...
- get_robot_detection(): 로봇이 감지한 재해 상황 정보를 가져옵니다 (연기, 화재, 쓰러진 사람, 응급상황).
  **중요**: detection 결과를 받으면 반드시 다음 형식으로 먼저 출력하세요:
  ```json
  {도구가 반환한 JSON 데이터를 그대로 출력}
  ```
  그 후 감지된 내용을 사용자 친화적으로 설명하세요.
- get_robot_gesture(): 로봇이 감지한 작업자의 제스처 정보를 가져옵니다.
  **중요**: gesture 결과를 받으면 반드시 다음 형식으로 먼저 출력하세요:
  ```json
  {도구가 반환한 JSON 데이터를 그대로 출력}
  ```
  그 후 감지된 제스처를 사용자 친화적으로 설명하세요.
...
2. 포인트 1에서 위험 상황 감지
   ...
   - **중요**: detection 결과를 받으면 반드시 다음 형식으로 출력하세요:
     ```json
     {도구가 반환한 JSON 데이터를 그대로 출력}
     ```
   - 그 후 감지된 위험 상황을 사용자 친화적으로 설명
...
4. 포인트 2에서 제스처 분석
   ...
   - **중요**: gesture 결과를 받으면 반드시 다음 형식으로 출력하세요:
     ```json
     {도구가 반환한 JSON 데이터를 그대로 출력}
     ```
   - 제스처가 감지된 경우:
   ...
   - 그 후 제스처 정보 및 로봇의 반응을 사용자 친화적으로 설명
